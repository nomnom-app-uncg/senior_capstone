...
export default function FridgeScreen() {
  ...
  const saveRecipe = async () => {
    ...
    try {
      // Use the first line as title (or customize as needed)
      const title = selectedDishRecipe.split("\n")[0] || "My Saved Recipe";
      const newRecipe = {
        id: Date.now().toString(),
        title,
        content: selectedDishRecipe,
      };

      const existing = await AsyncStorage.getItem("favoriteRecipes");
      ...
      // Emit an event so that the Profile screen can refresh favorites in real time
      DeviceEventEmitter.emit("favoritesUpdated");

      Alert.alert("Saved!", "Your recipe has been saved to favorites.");
    } catch (error) {
